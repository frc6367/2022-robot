...
class TwoBall(magicbot.AutonomousStateMachine):
    ...
    @magicbot.timed_state(duration=3.5, next_state="slightTurn")
    def forward(self):
        self.drivetrain.move(-0.3, 0)
        self.pointer.gotoAngle(179.9)

    @magicbot.timed_state(duration=1.5, next_state="shoot")
    def slightTurn(self):
        self.pointer.gotoAngle(155)
